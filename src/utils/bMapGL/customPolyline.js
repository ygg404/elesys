...
var polyLineObj = function () {
  ...
  polyLineObj.prototype.init = function (bPoint, vueObj) {
    ...
    this.polyLine = new BMapGL.Polyline(polyList, linestyle)
    ...
    this.label = new BMapGL.Label(bPoint.label, opts)  // 创建文本标注对象
    this.label.setStyle({
      color: 'red',
      fontSize: '12px',
      height: '20px',
  ...
  polyLineObj.prototype.createpolyLineLabel = function () {
    ...
    var html = '<div class=\'infoBoxContent \'><p>标题: ' + objbpoint.label +
      ...
      '</p></div></div>'
    this.label = new BMapGL.Label(html, opts) // 创建文本标注对象
    this.label.setStyle({
      opacity: '0.7',
      background: '#FFFAFA',
  ...
  polyLineObj.prototype.polyLineEvent = function () {
    ...
    this.polyLine.addEventListener('mouseover', function (e) {
      that.getlabel().setPosition(new BMapGL.Point(e.point.lng, e.point.lat))
      label.show()
    })
